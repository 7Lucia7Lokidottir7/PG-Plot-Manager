...
namespace PG.PlotManagement
{
    public partial class PGPlotControllerEditorWindow : EditorWindow
    {
        ...
        void OnConditionsMenu(int chapter, int state)
        {
            ...
            _conditionsCreateStateMenu = EditorGUILayout.BeginFoldoutHeaderGroup(_conditionsCreateStateMenu, "Create condition");
            ...
            for (int i = 0; i < plotState.plotConditions.Count; i++)
            {
                ...
                GUILayout.BeginVertical(box);
                plotState.plotConditions[i] = (PGPlotCondition)EditorGUILayout.ObjectField(plotState.plotConditions[i], typeof(PGPlotCondition), false); ;
